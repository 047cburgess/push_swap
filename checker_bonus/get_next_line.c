...
/*   Updated: 2025/01/10 16:40:31 by caburges         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_extract_line(char *vault)
{
	...
	new_line = my_calloc(line_len + 1, sizeof(char));
	if (!new_line)
		return (NULL);
	i = 0;
	while (i < line_len)
...
void	ft_set_new_start(char **vault)
{
	...
	if (*vault != NULL)
	{
		new_line_char_found = my_strchr(*vault, '\n');
		if (new_line_char_found)
			my_strcpy(*vault, new_line_char_found + 1);
		else
			(*vault)[0] = '\0';
	}
}
